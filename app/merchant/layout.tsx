...
export default function MerchantLayout({ children }: { children: React.ReactNode }) {
  ...
  const nav = [{ href: "/merchant", label: "Overview" }, { href: "/merchant/collect", label: "Collect" }];
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0f" }}>
      <header style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(13,13,15,0.8)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 50, padding: "0 24px", height: 56, display: "flex", alignItems: "center", gap: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 11, background: "rgba(124,106,247,0.15)", color: "#7c6af7", borderRadius: 6, padding: "2px 8px", marginLeft: 4, fontWeight: 500 }}>Merchant</span>
        </div>
        <nav style={{ display: "flex", gap: 4 }}>
          {nav.map(({ href, label }) => {
            const active = pathname === href;
            return <button key={href} onClick={() => router.push(href)} style={{ padding: "5px 12px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13.5, fontWeight: active ? 500 : 400, fontFamily: "inherit", background: active ? "rgba(124,106,247,0.15)" : "transparent", color: active ? "#7c6af7" : "rgba(240,239,244,0.5)" }}>{label}</button>;
          })}
        </nav>
